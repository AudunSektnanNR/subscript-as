...
def _extract_source_data(
    ...
    if _is_subset(["SGAS", "AMFG"], properties):  # NBNB-AS: Stemmer type her og under?
        ...
        error_text = "CO2 containment calculation failed. Cannot find necessary properties "
...
def _calculate_co2_from_source_data(
    ...
    if _is_subset(["SGAS"], active_props):
        ...
        if _is_subset(["PORV", "DGAS", "DWAT", "AMFG", "YMFG"], active_props):
            ...
            error_text = "Lacking some properties to compute CO2 mass/volume"
            raise RuntimeError(error_text)
    else:
        error_text = "Lacking some properties to compute CO2 mass/volume"
        error_text += "\nMissing: SGAS"
...
def calculate_co2(
    ...
    source_data = _extract_source_data(
        grid_file,
        unrst_file,
        PROPERTIES_USED_FOR_CALCULATIONS,
        init_file,
        zone_file
    )
